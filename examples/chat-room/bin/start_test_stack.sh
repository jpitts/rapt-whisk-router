...
CMD="bash -c 'sleep 2; screen -d $SCREENID' & screen -c $DIR/config/services.screenrc -m"
eval $CMD
...
CMD="screen -S $SCREENID -p0 -X stuff \$'\03 cat $DIR/config/services.txt \015'"
eval $CMD

echo "Done."
echo "Services are running in a screen session. Try this command to get started: "
