  ...
  */
  ...
  if (attr.sid) {
    ...
    Whisk.Models.Session({ sid: attr.sid }).read(null, function(err, session) { 
      if (err) {
        console.error("Whisk.Auth.get_session: Whisk.Models.Session could not read the session! " + err);
        return cb(err);
      } 
      
      return cb(null, { session: session });
      
    }); 

  // TODO: socket_id or user_id not yet implemented
  } else {
    var err = "Whisk.Auth.get_session: Finding session from socket_id or user_id not yet implemented!";
    console.error(err);
    return cb(err);
  }
    ...
    // success!
      ...
      client_ids.forEach(function (client_id, idx) { // (aside) engine client id should match the socket id
        ...
        if (client_id == attr.socket.id) {
          console.log('set sid and user_id in engine client ' + client_id);
