...
API.get_session = function (attr, cb) {
  ...
  */
  ...
  // get session using user_id
    ...
    // loop over all clients in the websocket service
    ...
    client_ids.forEach(function (client_id, idx) { // (aside) engine client id should match the socket id
      ...
      var client = Whisk.Config.io_instance().engine.clients[client_id];
      
      // matched session client via socket_id
      if (attr.socket_id) {
        //console.log(attr.socket_id + '==' + client.id);
        if (client && client_id == attr.socket_id) session_client = client;
      
      // matched session client via user_id
      } else if (attr.user_id) {
        //console.log(attr.user_id + '==' + client.whisk_user_id);
        if (client && client.whisk_user_id == attr.user_id) session_client = client;
      }

    });

    if (session_client) {
      Whisk.Models.Session({ sid: session_client.whisk_sid }).read(null, function(err, session) { 
        if (err) {
          console.error("Whisk.Auth.get_session: Whisk.Models.Session could not read the session with socket_id or user_id! " + err);
          return cb(err);
        } 
        if (!session) {
          err = "Whisk.Auth.get_session: Whisk.Models.Session could not find the session with socket_id or user_id!";
          console.error(err);
          return cb(err);
        }            
        return cb(null, session);
      ...
      var err = "Whisk.Auth.get_session: Could not find session!";
      console.error(err);
      return cb(err);
