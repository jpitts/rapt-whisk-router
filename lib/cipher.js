...
API.tenent_handler = function tenentHandler (recipient, cb) {
  ...
  */
  ...
  if (recipient && recipient.tid && Whisk.Config.io_instance().sockets.connected[recipient.tid]) {        
    console.log('recipient.tid=' + recipient.tid);
    cb(null, Whisk.Config.io_instance().sockets.connected[recipient.tid]);
