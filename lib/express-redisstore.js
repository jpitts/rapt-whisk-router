...
Client.prototype.get = function (fn) {
	this.store.cmd.get(this.id, function(err, data){
		try {
			if (!data) return fn();
			data = data.toString();
...
Client.prototype.set = function(sess, fn) {
	try {
		var maxAge = sess.cookie.originalMaxAge
			, ttl = Math.floor(maxAge / 1000)
			, sess = JSON.stringify(sess);

		this.store.cmd.setex(this.id, ttl, sess, function(err){
